...
cLCMhalfnormlike5C <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  ...
  phi2 <- parm[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar)]
  beta3 <- parm[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 *
    nXvar + 2 * nuHvar + 2 * nvHvar)]
  delta3 <- parm[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 *
    nXvar + 3 * nuHvar + 2 * nvHvar)]
  phi3 <- parm[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 *
    nXvar + 3 * nuHvar + 3 * nvHvar)]
  beta4 <- parm[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 *
    nXvar + 3 * nuHvar + 3 * nvHvar)]
  delta4 <- parm[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 *
    nXvar + 4 * nuHvar + 3 * nvHvar)]
  phi4 <- parm[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 *
    nXvar + 4 * nuHvar + 4 * nvHvar)]
  beta5 <- parm[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 *
    nXvar + 4 * nuHvar + 4 * nvHvar)]
  delta5 <- parm[(5 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 *
    nXvar + 5 * nuHvar + 4 * nvHvar)]
  phi5 <- parm[(5 * nXvar + 5 * nuHvar + 4 * nvHvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar)]
  theta1 <- parm[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)]
  theta2 <- parm[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar +
    1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar)]
  theta3 <- parm[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 2 *
    nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 *
    nZvar)]
  theta4 <- parm[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 *
    nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 4 *
    nZvar)]
...
csLCMfhalfnorm5C <- function(olsObj, epsiRes, nXvar, nuHvar,
  ...
  StartVal <- c(Esti[1:(nXvar + 1)], if (nuHvar > 1) rep(0,
    nuHvar - 1), Esti[nXvar + 2], if (nvHvar > 1) rep(0,
    nvHvar - 1), 0.98 * Esti[1:nXvar], Esti[nXvar + 1], if (nuHvar >
    ...
    2], if (nvHvar > 1) rep(0, nvHvar - 1), 0.98 * Esti[1:nXvar],
    Esti[nXvar + 1], if (nuHvar > 1) rep(0, nuHvar - 1),
    Esti[nXvar + 2], if (nvHvar > 1) rep(0, nvHvar - 1),
    0.98 * Esti[1:nXvar], Esti[nXvar + 1], if (nuHvar > 1) rep(0,
      nuHvar - 1), Esti[nXvar + 2], if (nvHvar > 1) rep(0,
      nvHvar - 1), rep(0, 4 * nZvar))
...
cgradLCMhalfnormlike5C <- function(parm, nXvar, nuHvar, nvHvar,
...
chessLCMhalfnormlike5C <- function(parm, nXvar, nuHvar, nvHvar,
  uHvar, vHvar, Yvar, Xvar, S, Zvar, nZvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi1 <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
  ...
  hessll <- matrix(nrow = (5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    4 * nZvar), ncol = (5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    4 * nZvar))
  ...
  hessll[1:nXvar, (nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = 2 * (S * (((S * (sigx10_1 + S * sigx3_1 *
      dmusig1 * (duv1) * ewu1 * (epsilon1)/sqsq1^2) * (epsilon1) -
      0.5 * (sigx1_1/(sigma_sq1)))/(sigma_sq1) - sigx3_1 *
      dmusig1 * depsisq1 * ewu1/sqsq1^2)/(sigwz1) - 2 *
      (sigx1_1 * ewz1 * sigx7_1/((sigwz1)^2 * (sigma_sq1)))) *
      ewv1 * ewz1), FUN = "*"), vHvar)
  ...
  hessll[1:nXvar, (2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar +
    ...
    STATS = -(4 * (S * sigx1_1 * ewu2 * ewz1 * ewz2 * sigx6_2 *
      sqrt(sigma_sq2)/((sigwz2)^2 * (sigma_sq1)^(3/2)))),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 *
    ...
    FUN = "*"), vHvar)
  hessll[1:nXvar, (2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 *
    nXvar + 2 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S^2 * sigx1_1 * sigx1_3 *
      (sigma_sq3) * ewz1 * ewz3 * sqrt(sigma_sq3)/(sigsq_3^2 *
      (sigma_sq1)^(3/2)))), FUN = "*"), Xvar)
  hessll[1:nXvar, (3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 *
    nXvar + 3 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewu3 * ewz1 *
      ewz3 * sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 * (sigma_sq1)^(3/2)))),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 *
    nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewv3 * ewz1 *
      ewz3 * sigx7_3 * sqrt(sigma_sq3)/((sigwz3)^2 * (sigma_sq1)^(3/2)))),
    FUN = "*"), vHvar)
  hessll[1:nXvar, (3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 *
    nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S^2 * sigx1_1 * sigx1_4 *
      (sigma_sq4) * ewz1 * ewz4 * sqrt(sigma_sq4)/(sigsq_4^2 *
      (sigma_sq1)^(3/2)))), FUN = "*"), Xvar)
  hessll[1:nXvar, (4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 *
    nXvar + 4 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewu4 * ewz1 *
      ewz4 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 * (sigma_sq1)^(3/2)))),
    FUN = "*"), uHvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 *
    nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_1 * ewv4 * ewz1 *
      ewz4 * sigx7_4 * sqrt(sigma_sq4)/((sigwz4)^2 * (sigma_sq1)^(3/2)))),
    FUN = "*"), vHvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 *
    nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S^2 * prC * sigx1_1 * sigx1_5 *
      (sigma_sq5) * ewz1 * sqrt(sigma_sq5)/(wzc1 * (sigma_sq1)^(3/2)))),
    FUN = "*"), Xvar)
  hessll[1:nXvar, (5 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 *
    nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_1 * ewu5 *
      ewz1 * sigx6_5 * sqrt(sigma_sq5)/wzsigx4_1)), FUN = "*"),
    uHvar)
  hessll[1:nXvar, (5 * nXvar + 5 * nuHvar + 4 * nvHvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_1 * ewv5 *
      ewz1 * sigx7_5 * sqrt(sigma_sq5)/wzsigx4_1)), FUN = "*"),
    vHvar)
  hessll[1:nXvar, (5 * nXvar + 5 * nuHvar + 5 * nvHvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * sigx4w5z * sigx1_1 * ewz1/((sigma_sq1)^(3/2)),
    FUN = "*"), Zvar)
  hessll[1:nXvar, (5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar +
    1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4w2z * sigx1_1 * ewz1 *
      ewz2/((sigma_sq1)^(3/2))), FUN = "*"), Zvar)
  hessll[1:nXvar, (5 * nXvar + 5 * nuHvar + 5 * nvHvar + 2 *
    nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 *
    nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx4w3z * sigx1_1 * ewz1 * ewz3/((sigma_sq1)^(3/2))),
    FUN = "*"), Zvar)
  hessll[1:nXvar, (5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 *
    nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 4 *
    nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
  ...
  hessll[(nXvar + 1):(nXvar + nuHvar), (nXvar + nuHvar + 1):(nXvar +
    ...
    STATS = 2 * (((S * (((((0.5 * ((wusq1) * ewv1) - S^2 *
      sigx3_1 * sqewu1 * ewu1 * (epsilon1)^2)/(sigma_sq1) +
  ...
  hessll[(nXvar + 1):(nXvar + nuHvar), (2 * nXvar + 2 * nuHvar +
    2 * nvHvar + 1):(3 * nXvar + 2 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * ewu1 * (sigma_sq3) *
      ewz1 * ewz3 * sigx6_1 * sqrt(sigma_sq3)/(sigsq_3^2 *
      sqrt(sigma_sq1)))), FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (3 * nXvar + 2 * nuHvar +
    2 * nvHvar + 1):(3 * nXvar + 3 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu1 * ewu3 * ewz1 * ewz3 *
      sigx6_1 * sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 *
      sqrt(sigma_sq1)))), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (3 * nXvar + 3 * nuHvar +
    2 * nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu1 * ewv3 * ewz1 * ewz3 *
      sigx7_3 * sigx6_1 * sqrt(sigma_sq3)/((sigwz3)^2 *
      sqrt(sigma_sq1)))), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (3 * nXvar + 3 * nuHvar +
    3 * nvHvar + 1):(4 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * ewu1 * (sigma_sq4) *
      ewz1 * ewz4 * sigx6_1 * sqrt(sigma_sq4)/(sigsq_4^2 *
      sqrt(sigma_sq1)))), FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (4 * nXvar + 3 * nuHvar +
    3 * nvHvar + 1):(4 * nXvar + 4 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu1 * ewu4 * ewz1 * ewz4 *
      sigx6_1 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq1)))), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (4 * nXvar + 4 * nuHvar +
    3 * nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu1 * ewv4 * ewz1 * ewz4 *
      sigx7_4 * sigx6_1 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq1)))), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (4 * nXvar + 4 * nuHvar +
    4 * nvHvar + 1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_5 * ewu1 *
      (sigma_sq5) * ewz1 * sigx6_1 * sqrt(sigma_sq5)/(sigsq_5^2 *
      wzdsq1))), FUN = "*"), Xvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (5 * nXvar + 4 * nuHvar +
    4 * nvHvar + 1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu1 * ewu5 * ewz1 *
      sigx6_1 * sigx6_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq1))), FUN = "*"), uHvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (5 * nXvar + 5 * nuHvar +
    4 * nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu1 * ewv5 * ewz1 *
      sigx7_5 * sigx6_1 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq1))), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = sigx4w5z *
    ewu1 * ewz1 * sigx6_1/sqrt(sigma_sq1), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (5 * nXvar + 5 * nuHvar +
    5 * nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(sigx4w2z * ewu1 * ewz1 * ewz2 * sigx6_1/sqrt(sigma_sq1)),
    FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 3 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(sigx4w3z * ewu1 * ewz1 * ewz3 * sigx6_1/sqrt(sigma_sq1)),
    FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 4 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(sigx4w4z * ewu1 * ewz1 * ewz4 * sigx6_1/sqrt(sigma_sq1)),
    FUN = "*"), Zvar)
  ...
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (nXvar +
    ...
    MARGIN = 1, STATS = -(4 * (S * sigx1_2 * (sigma_sq2) *
      ewv1 * ewz1 * ewz2 * sigx7_1 * sqrt(sigma_sq2)/(sigsq_2^2 *
      sqrt(sigma_sq1)))), FUN = "*"), Xvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (2 *
    ...
    (ewu2 * ewv1 * ewz1 * ewz2 * sigx7_1 * sigx6_2 * sqrt(sigma_sq2)/((sigwz2)^2 *
      sqrt(sigma_sq1)))), FUN = "*"), uHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (2 *
    nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar + 2 * nuHvar +
    2 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(4 *
    (ewv1 * ewv2 * ewz1 * ewz2 * sigx7_1 * sigx7_2 * sqrt(sigma_sq2)/((sigwz2)^2 *
      sqrt(sigma_sq1)))), FUN = "*"), vHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (2 *
    nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar + 2 *
    nuHvar + 2 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (S * sigx1_3 * (sigma_sq3) * ewv1 * ewz1 *
      ewz3 * sigx7_1 * sqrt(sigma_sq3)/(sigsq_3^2 * sqrt(sigma_sq1)))),
    FUN = "*"), Xvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (3 *
    nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar + 3 *
    nuHvar + 2 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (ewu3 * ewv1 * ewz1 * ewz3 * sigx7_1 *
      sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 * sqrt(sigma_sq1)))),
    FUN = "*"), uHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (3 *
    nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar + 3 *
    nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (ewv1 * ewv3 * ewz1 * ewz3 * sigx7_1 *
      sigx7_3 * sqrt(sigma_sq3)/((sigwz3)^2 * sqrt(sigma_sq1)))),
    FUN = "*"), vHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (3 *
    nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar + 3 *
    nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (S * sigx1_4 * (sigma_sq4) * ewv1 * ewz1 *
      ewz4 * sigx7_1 * sqrt(sigma_sq4)/(sigsq_4^2 * sqrt(sigma_sq1)))),
    FUN = "*"), Xvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (4 *
    nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar + 4 *
    nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (ewu4 * ewv1 * ewz1 * ewz4 * sigx7_1 *
      sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 * sqrt(sigma_sq1)))),
    FUN = "*"), uHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (4 *
    nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar + 4 *
    nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (ewv1 * ewv4 * ewz1 * ewz4 * sigx7_1 *
      sigx7_4 * sqrt(sigma_sq4)/((sigwz4)^2 * sqrt(sigma_sq1)))),
    FUN = "*"), vHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar + 4 *
    nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (S * prC * sigx1_5 * (sigma_sq5) * ewv1 *
      ewz1 * sigx7_1 * sqrt(sigma_sq5)/(sigsq_5^2 * wzdsq1))),
    FUN = "*"), Xvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (5 *
    nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar + 5 *
    nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (prC * ewu5 * ewv1 * ewz1 * sigx7_1 * sigx6_5 *
      sqrt(sigma_sq5)/((sigwz5)^2 * wzdsq1))), FUN = "*"),
    uHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (5 *
    nXvar + 5 * nuHvar + 4 * nvHvar + 1):(5 * nXvar + 5 *
    nuHvar + 5 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (prC * ewv1 * ewv5 * ewz1 * sigx7_1 * sigx7_5 *
      sqrt(sigma_sq5)/((sigwz5)^2 * wzdsq1))), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + 1):(5 * nXvar + 5 *
    nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx4w5z * ewv1 * ewz1 * sigx7_1/sqrt(sigma_sq1),
    FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + nZvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar + 2 * nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4w2z * ewv1 * ewz1 * ewz2 *
      sigx7_1/sqrt(sigma_sq1)), FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4w3z * ewv1 * ewz1 * ewz3 *
      sigx7_1/sqrt(sigma_sq1)), FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + 3 * nZvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + 4 * nZvar)] <- crossprod(sweep(vHvar,
  ...
  hessll[(nXvar + nuHvar + nvHvar + 1):(2 * nXvar + nuHvar +
    ...
    (S^2 * ((depsisq2 * (S * (dmusig2 * ewu2/sigmastar2 +
  ...
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    ...
    1):(3 * nXvar + 2 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * ewu2 * (sigma_sq3) *
      ewz2 * ewz3 * sigx6_2 * sqrt(sigma_sq3)/(sigsq_3^2 *
      sqrt(sigma_sq2)))), FUN = "*"), Xvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (3 * nXvar + 2 * nuHvar + 2 * nvHvar +
    1):(3 * nXvar + 3 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu2 * ewu3 * ewz2 * ewz3 *
      sigx6_2 * sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 *
      sqrt(sigma_sq2)))), FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (3 * nXvar + 3 * nuHvar + 2 * nvHvar +
    1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu2 * ewv3 * ewz2 * ewz3 *
      sigx7_3 * sigx6_2 * sqrt(sigma_sq3)/((sigwz3)^2 *
      sqrt(sigma_sq2)))), FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (3 * nXvar + 3 * nuHvar + 3 * nvHvar +
    1):(4 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * ewu2 * (sigma_sq4) *
      ewz2 * ewz4 * sigx6_2 * sqrt(sigma_sq4)/(sigsq_4^2 *
      sqrt(sigma_sq2)))), FUN = "*"), Xvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (4 * nXvar + 3 * nuHvar + 3 * nvHvar +
    1):(4 * nXvar + 4 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu2 * ewu4 * ewz2 * ewz4 *
      sigx6_2 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq2)))), FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (4 * nXvar + 4 * nuHvar + 3 * nvHvar +
    1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu2 * ewv4 * ewz2 * ewz4 *
      sigx7_4 * sigx6_2 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq2)))), FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_5 * ewu2 *
      (sigma_sq5) * ewz2 * sigx6_2 * sqrt(sigma_sq5)/(sigsq_5^2 *
      wzdsq2))), FUN = "*"), Xvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (5 * nXvar + 4 * nuHvar + 4 * nvHvar +
    1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu2 * ewu5 * ewz2 *
      sigx6_2 * sigx6_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq2))), FUN = "*"), uHvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (5 * nXvar + 5 * nuHvar + 4 * nvHvar +
    1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu2 * ewv5 * ewz2 *
      sigx7_5 * sigx6_2 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq2))), FUN = "*"), vHvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx4w1z * ewu2 * ewz1 * ewz2 *
      sigx6_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 2 *
    nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = sigx4w6z *
    ewu2 * ewz2 * sigx6_2/sqrt(sigma_sq2), FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    3 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx4w3z *
    ewu2 * ewz2 * ewz3 * sigx6_2/sqrt(sigma_sq2)), FUN = "*"),
    Zvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    4 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx4w4z *
  ...
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    ...
    nvHvar + 1):(3 * nXvar + 2 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * (sigma_sq3) *
      ewv2 * ewz2 * ewz3 * sigx7_2 * sqrt(sigma_sq3)/(sigsq_3^2 *
      sqrt(sigma_sq2)))), FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 2 * nuHvar + 2 *
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewu3 * ewv2 * ewz2 * ewz3 *
      sigx7_2 * sigx6_3 * sqrt(sigma_sq3)/((sigwz3)^2 *
      sqrt(sigma_sq2)))), FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 2 *
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewv2 * ewv3 * ewz2 * ewz3 *
      sigx7_2 * sigx7_3 * sqrt(sigma_sq3)/((sigwz3)^2 *
      sqrt(sigma_sq2)))), FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * (sigma_sq4) *
      ewv2 * ewz2 * ewz4 * sigx7_2 * sqrt(sigma_sq4)/(sigsq_4^2 *
      sqrt(sigma_sq2)))), FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewu4 * ewv2 * ewz2 * ewz4 *
      sigx7_2 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq2)))), FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewv2 * ewv4 * ewz2 * ewz4 *
      sigx7_2 * sigx7_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq2)))), FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_5 * (sigma_sq5) *
      ewv2 * ewz2 * sigx7_2 * sqrt(sigma_sq5)/(sigsq_5^2 *
      wzdsq2))), FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu5 * ewv2 * ewz2 *
      sigx7_2 * sigx6_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq2))), FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewv2 * ewv5 * ewz2 *
      sigx7_2 * sigx7_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq2))), FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4w1z * ewv2 * ewz1 * ewz2 *
      sigx7_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = sigx4w6z *
    ewv2 * ewz2 * sigx7_2/sqrt(sigma_sq2), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(sigx4w3z * ewv2 * ewz2 * ewz3 * sigx7_2/sqrt(sigma_sq2)),
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 4 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(sigx4w4z * ewv2 * ewz2 * ewz4 * sigx7_2/sqrt(sigma_sq2)),
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (2 * nXvar + 2 * nuHvar + 2 *
    nvHvar + 1):(3 * nXvar + 2 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(Xvar,
  ...
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 2 * nuHvar + 2 *
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(Xvar,
  ...
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 2 *
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(Xvar,
  ...
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S^2 * sigx1_3 * sigx1_4 *
      (sigma_sq4) * ewz3 * ewz4 * sqrt(sigma_sq4)/(sigsq_4^2 *
      (sigma_sq3)^(3/2)))), FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * ewu4 * ewz3 *
      ewz4 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 * (sigma_sq3)^(3/2)))),
    FUN = "*"), uHvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_3 * ewv4 * ewz3 *
      ewz4 * sigx7_4 * sqrt(sigma_sq4)/((sigwz4)^2 * (sigma_sq3)^(3/2)))),
    FUN = "*"), vHvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S^2 * prC * sigx1_3 * sigx1_5 *
      (sigma_sq5) * ewz3 * sqrt(sigma_sq5)/(wzc1 * (sigma_sq3)^(3/2)))),
    FUN = "*"), Xvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_3 * ewu5 *
      ewz3 * sigx6_5 * sqrt(sigma_sq5)/wzsigx4_3)), FUN = "*"),
    uHvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_3 * ewv5 *
      ewz3 * sigx7_5 * sqrt(sigma_sq5)/wzsigx4_3)), FUN = "*"),
    vHvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4w1z * sigx1_3 * ewz1 *
      ewz3/((sigma_sq3)^(3/2))), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx4w2z * sigx1_3 * ewz2 * ewz3/((sigma_sq3)^(3/2))),
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = S * sigx4w7z * sigx1_3 * ewz3/((sigma_sq3)^(3/2)),
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 4 * nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1,
    STATS = -(S * sigx4w4z * sigx1_3 * ewz3 * ewz4/((sigma_sq3)^(3/2))),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (3 * nXvar + 2 * nuHvar + 2 *
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 2 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((ewu3 * (S * (0.5 * sigx15_3 -
      ...
      ewu3 * ewz3), FUN = "*"), uHvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 2 *
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
  ...
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * ewu3 * (sigma_sq4) *
      ewz3 * ewz4 * sigx6_3 * sqrt(sigma_sq4)/(sigsq_4^2 *
      sqrt(sigma_sq3)))), FUN = "*"), Xvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (4 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu3 * ewu4 * ewz3 * ewz4 *
      sigx6_3 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq3)))), FUN = "*"), uHvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (ewu3 * ewv4 * ewz3 * ewz4 *
      sigx7_4 * sigx6_3 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq3)))), FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_5 * ewu3 *
      (sigma_sq5) * ewz3 * sigx6_3 * sqrt(sigma_sq5)/(sigsq_5^2 *
      wzdsq3))), FUN = "*"), Xvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (5 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu3 * ewu5 * ewz3 *
      sigx6_3 * sigx6_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq3))), FUN = "*"), uHvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu3 * ewv5 * ewz3 *
      sigx7_5 * sigx6_3 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq3))), FUN = "*"), vHvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx4w1z * ewu3 * ewz1 * ewz3 *
      sigx6_3/sqrt(sigma_sq3)), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx4w2z *
    ewu3 * ewz2 * ewz3 * sigx6_3/sqrt(sigma_sq3)), FUN = "*"),
    Zvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = sigx4w7z * ewu3 * ewz3 * sigx6_3/sqrt(sigma_sq3),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 4 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(sigx4w4z * ewu3 * ewz3 * ewz4 * sigx6_3/sqrt(sigma_sq3)),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (3 * nXvar + 3 * nuHvar + 2 *
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar,
    ...
    vHvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * sigx1_4 * (sigma_sq4) *
      ewv3 * ewz3 * ewz4 * sigx7_3 * sqrt(sigma_sq4)/(sigsq_4^2 *
      sqrt(sigma_sq3)))), FUN = "*"), Xvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (4 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewu4 * ewv3 * ewz3 * ewz4 *
      sigx7_3 * sigx6_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq3)))), FUN = "*"), uHvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (ewv3 * ewv4 * ewz3 * ewz4 *
      sigx7_3 * sigx7_4 * sqrt(sigma_sq4)/((sigwz4)^2 *
      sqrt(sigma_sq3)))), FUN = "*"), vHvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_5 * (sigma_sq5) *
      ewv3 * ewz3 * sigx7_3 * sqrt(sigma_sq5)/(sigsq_5^2 *
      wzdsq3))), FUN = "*"), Xvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu5 * ewv3 * ewz3 *
      sigx7_3 * sigx6_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq3))), FUN = "*"), uHvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewv3 * ewv5 * ewz3 *
      sigx7_3 * sigx7_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq3))), FUN = "*"), vHvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4w1z * ewv3 * ewz1 * ewz3 *
      sigx7_3/sqrt(sigma_sq3)), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx4w2z *
    ewv3 * ewz2 * ewz3 * sigx7_3/sqrt(sigma_sq3)), FUN = "*"),
    Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = sigx4w7z * ewv3 * ewz3 * sigx7_3/sqrt(sigma_sq3),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 4 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(sigx4w4z * ewv3 * ewz3 * ewz4 * sigx7_3/sqrt(sigma_sq3)),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    3 * nuHvar + 3 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 3 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(Xvar,
  ...
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4w1z * sigx1_4 * ewz1 *
      ewz4/((sigma_sq4)^(3/2))), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx4w2z * sigx1_4 * ewz2 * ewz4/((sigma_sq4)^(3/2))),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    3 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
  ...
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    ...
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (4 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 3 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((ewu4 * (S * (0.5 * sigx15_4 -
      ...
      ewu4 * ewz4), FUN = "*"), uHvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
  ...
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_5 * ewu4 *
      (sigma_sq5) * ewz4 * sigx6_4 * sqrt(sigma_sq5)/(sigsq_5^2 *
      wzdsq4))), FUN = "*"), Xvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (5 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu4 * ewu5 * ewz4 *
      sigx6_4 * sigx6_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq4))), FUN = "*"), uHvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu4 * ewv5 * ewz4 *
      sigx7_5 * sigx6_4 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq4))), FUN = "*"), vHvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx4w1z * ewu4 * ewz1 * ewz4 *
      sigx6_4/sqrt(sigma_sq4)), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx4w2z *
    ewu4 * ewz2 * ewz4 * sigx6_4/sqrt(sigma_sq4)), FUN = "*"),
    Zvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(sigx4w3z * ewu4 * ewz3 * ewz4 * sigx6_4/sqrt(sigma_sq4)),
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 4 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = sigx4w8z * ewu4 * ewz4 * sigx6_4/sqrt(sigma_sq4),
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (4 * nXvar + 4 * nuHvar + 3 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    ...
    vHvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (S * prC * sigx1_5 * (sigma_sq5) *
      ewv4 * ewz4 * sigx7_4 * sqrt(sigma_sq5)/(sigsq_5^2 *
      wzdsq4))), FUN = "*"), Xvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewu5 * ewv4 * ewz4 *
      sigx7_4 * sigx6_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq4))), FUN = "*"), uHvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(4 * (prC * ewv4 * ewv5 * ewz4 *
      sigx7_4 * sigx7_5 * sqrt(sigma_sq5)/((sigwz5)^2 *
      wzdsq4))), FUN = "*"), vHvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4w1z * ewv4 * ewz1 * ewz4 *
      sigx7_4/sqrt(sigma_sq4)), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx4w2z *
    ewv4 * ewz2 * ewz4 * sigx7_4/sqrt(sigma_sq4)), FUN = "*"),
    Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(sigx4w3z * ewv4 * ewz3 * ewz4 * sigx7_4/sqrt(sigma_sq4)),
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 4 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = sigx4w8z * ewv4 * ewz4 * sigx7_4/sqrt(sigma_sq4),
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    4 * nuHvar + 4 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 4 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(Xvar,
  ...
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * prC * sigx4w9z * sigx1_5 *
      ewz1/((sigma_sq5)^(3/2))), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    prC * sigx4w10z * sigx1_5 * ewz2/((sigma_sq5)^(3/2))),
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    4 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
  ...
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    ...
    FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 4 * nvHvar), (5 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 4 * nvHvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = 2 * (((ewu5 * (S * (0.5 * sigx15_5 -
      ...
      prC * ewu5), FUN = "*"), uHvar)
  hessll[(5 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(uHvar,
  ...
  hessll[(5 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(prC * sigx4w9z * ewu5 * ewz1 *
      sigx6_5/sqrt(sigma_sq5)), FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(prC *
    sigx4w10z * ewu5 * ewz2 * sigx6_5/sqrt(sigma_sq5)), FUN = "*"),
    Zvar)
  hessll[(5 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(prC * sigx4w11z * ewu5 * ewz3 * sigx6_5/sqrt(sigma_sq5)),
    FUN = "*"), Zvar)
  hessll[(5 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 4 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 4 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(prC * sigx4w12z * ewu5 * ewz4 * sigx6_5/sqrt(sigma_sq5)),
    FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar), (5 * nXvar + 5 * nuHvar + 4 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar)] <- crossprod(sweep(vHvar,
    ...
    vHvar)
  hessll[(5 * nXvar + 5 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(prC * sigx4w9z * ewv5 * ewz1 *
      sigx7_5/sqrt(sigma_sq5)), FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(prC *
    sigx4w10z * ewv5 * ewz2 * sigx7_5/sqrt(sigma_sq5)), FUN = "*"),
    Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(prC * sigx4w11z * ewv5 * ewz3 * sigx7_5/sqrt(sigma_sq5)),
    FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 4 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar), (5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 4 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(prC * sigx4w12z * ewv5 * ewz4 * sigx7_5/sqrt(sigma_sq5)),
    FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar + nZvar), (5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar +
    nZvar)] <- crossprod(sweep(Zvar, MARGIN = 1, STATS = ((1 -
    ewz1/wzdeno)/wzdsig - 2 * (dwp1/dwz1)) * sigx4w15z *
    ewz1, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar + nZvar), (5 * nXvar + 5 * nuHvar +
    5 * nvHvar + nZvar + 1):(5 * nXvar + 5 * nuHvar + 5 *
    nvHvar + 2 * nZvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = -((sigx4w15z/(sigx4wzsq) + 2 * (sigx4w16z * dwzep1)) *
      ewz1 * ewz2), FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar + nZvar), (5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 2 * nZvar + 1):(5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 3 * nZvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = -((sigx4w15z/(sigx4wzsq) + 2 * (sigx4w17z * dwzep1)) *
      ewz1 * ewz3), FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar + nZvar), (5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 3 * nZvar + 1):(5 * nXvar + 5 * nuHvar +
    5 * nvHvar + 4 * nZvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = -((sigx4w15z/(sigx4wzsq) + 2 * (sigx4w18z * dwzep1)) *
      ewz1 * ewz4), FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar), (5 * nXvar +
    5 * nuHvar + 5 * nvHvar + nZvar + 1):(5 * nXvar + 5 *
    nuHvar + 5 * nvHvar + 2 * nZvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = ((1 - ewz2/wzdeno)/wzdsig - 2 * (dwp2/dwz2)) *
      sigx4w16z * ewz2, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar), (5 * nXvar +
    5 * nuHvar + 5 * nvHvar + 2 * nZvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar + 3 * nZvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -((sigx4w16z/(sigx4wzsq) + 2 * (sigx4w17z *
      dwzep2)) * ewz2 * ewz3), FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + nZvar + 1):(5 *
    nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar), (5 * nXvar +
    5 * nuHvar + 5 * nvHvar + 3 * nZvar + 1):(5 * nXvar +
    5 * nuHvar + 5 * nvHvar + 4 * nZvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -((sigx4w16z/(sigx4wzsq) + 2 * (sigx4w18z *
      dwzep2)) * ewz2 * ewz4), FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar +
    1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 * nZvar),
    (5 * nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar + 1):(5 *
      nXvar + 5 * nuHvar + 5 * nvHvar + 3 * nZvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = ((1 - ewz3/wzdeno)/wzdsig - 2 * (dwp3/dwz3)) *
      sigx4w17z * ewz3, FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 2 * nZvar +
    1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 * nZvar),
    (5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 * nZvar + 1):(5 *
      nXvar + 5 * nuHvar + 5 * nvHvar + 4 * nZvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -((sigx4w17z/(sigx4wzsq) + 2 * (sigx4w18z *
      dwzep3)) * ewz3 * ewz4), FUN = "*"), Zvar)
  hessll[(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 * nZvar +
    1):(5 * nXvar + 5 * nuHvar + 5 * nvHvar + 4 * nZvar),
    (5 * nXvar + 5 * nuHvar + 5 * nvHvar + 3 * nZvar + 1):(5 *
      nXvar + 5 * nuHvar + 5 * nvHvar + 4 * nZvar)] <- crossprod(sweep(Zvar,
...
LCM5ChnormAlgOpt <- function(start, olsParam, dataTable, S, nXvar,
  ...
  start_st <- if (!is.null(start))
    start else csLCMfhalfnorm5C(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  if (initStart) {
    ...
    initModel <- nlminb(start = startVal, objective = function(parm) -sum(cLCMhalfnormlike5C(parm,
      ...
      S = S, Zvar = Zvar, nZvar = nZvar)), gradient = function(parm) -colSums(cgradLCMhalfnormlike5C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, Zvar = Zvar, nZvar = nZvar)), hessian = function(parm) -chessLCMhalfnormlike5C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
  ...
  mleObj <- switch(method, ucminf = ucminf(par = startVal,
    fn = function(parm) -sum(cLCMhalfnormlike5C(parm, nXvar = nXvar,
      ...
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, Zvar = Zvar,
      nZvar = nZvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike5C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    ...
    maxLikAlgo = maxRoutine(fn = cLCMhalfnormlike5C, grad = cgradLCMhalfnormlike5C,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike5C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike5C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike5C(parm,
        ...
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), hs = function(parm) as(-chessLCMhalfnormlike5C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike5C(parm,
        ...
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), hess = function(parm) -chessLCMhalfnormlike5C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
  ...
  mleObj$logL_OBS <- cLCMhalfnormlike5C(parm = mleParam, nXvar = nXvar,
    nuHvar = nuHvar, nvHvar = nvHvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, Zvar = Zvar, nZvar = nZvar)
  mleObj$gradL_OBS <- cgradLCMhalfnormlike5C(parm = mleParam,
    nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, Zvar = Zvar,
    nZvar = nZvar)
...
cLCM5Chalfnormeff <- function(object, level) {
  ...
  phi2 <- object$mleParam[(2 * object$nXvar + 2 * object$nuHvar +
    object$nvHvar + 1):(2 * object$nXvar + 2 * object$nuHvar +
    2 * object$nvHvar)]
  beta3 <- object$mleParam[(2 * object$nXvar + 2 * object$nuHvar +
    2 * object$nvHvar + 1):(3 * object$nXvar + 2 * object$nuHvar +
    2 * object$nvHvar)]
  delta3 <- object$mleParam[(3 * object$nXvar + 2 * object$nuHvar +
    2 * object$nvHvar + 1):(3 * object$nXvar + 3 * object$nuHvar +
    2 * object$nvHvar)]
  phi3 <- object$mleParam[(3 * object$nXvar + 3 * object$nuHvar +
    2 * object$nvHvar + 1):(3 * object$nXvar + 3 * object$nuHvar +
    3 * object$nvHvar)]
  beta4 <- object$mleParam[(3 * object$nXvar + 3 * object$nuHvar +
    3 * object$nvHvar + 1):(4 * object$nXvar + 3 * object$nuHvar +
    3 * object$nvHvar)]
  delta4 <- object$mleParam[(4 * object$nXvar + 3 * object$nuHvar +
    3 * object$nvHvar + 1):(4 * object$nXvar + 4 * object$nuHvar +
    3 * object$nvHvar)]
  phi4 <- object$mleParam[(4 * object$nXvar + 4 * object$nuHvar +
    3 * object$nvHvar + 1):(4 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar)]
  beta5 <- object$mleParam[(4 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar + 1):(5 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar)]
  delta5 <- object$mleParam[(5 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar + 1):(5 * object$nXvar + 5 * object$nuHvar +
    4 * object$nvHvar)]
  phi5 <- object$mleParam[(5 * object$nXvar + 5 * object$nuHvar +
    4 * object$nvHvar + 1):(5 * object$nXvar + 5 * object$nuHvar +
    5 * object$nvHvar)]
  theta1 <- object$mleParam[(5 * object$nXvar + 5 * object$nuHvar +
    5 * object$nvHvar + 1):(5 * object$nXvar + 5 * object$nuHvar +
    5 * object$nvHvar + object$nZvar)]
  theta2 <- object$mleParam[(5 * object$nXvar + 5 * object$nuHvar +
    5 * object$nvHvar + object$nZvar + 1):(5 * object$nXvar +
    5 * object$nuHvar + 5 * object$nvHvar + 2 * object$nZvar)]
  theta3 <- object$mleParam[(5 * object$nXvar + 5 * object$nuHvar +
    5 * object$nvHvar + 2 * object$nZvar + 1):(5 * object$nXvar +
    5 * object$nuHvar + 5 * object$nvHvar + 3 * object$nZvar)]
  theta4 <- object$mleParam[(5 * object$nXvar + 5 * object$nuHvar +
    5 * object$nvHvar + 3 * object$nZvar + 1):(5 * object$nXvar +
    5 * object$nuHvar + 5 * object$nvHvar + 4 * object$nZvar)]
  ...
  Pcond_c5 <- Pi5 * Probc5/(Pi1 * Probc1 + Pi2 * Probc2 + Pi3 *
    Probc3 + Pi4 * Probc4 + Pi5 * Probc5)
  Group_c <- which.max(c(Pcond_c1, Pcond_c2, Pcond_c3, Pcond_c4,
    Pcond_c5))
  ...
  if (object$logDepVar == TRUE) {
    ...
    res <- bind_cols(u_c = u_c, teJLMS_c = teJLMS_c, PosteriorProb_c = P_cond_c,
      ...
      u_c4 = u_c4, u_c5 = u_c5, PosteriorProb_c1 = Pcond_c1,
      PosteriorProb_c2 = Pcond_c2, PosteriorProb_c3 = Pcond_c3,
      PosteriorProb_c4 = Pcond_c4, PosteriorProb_c5 = Pcond_c5,
      PriorProb_c1 = Probc1, PriorProb_c2 = Probc2, PriorProb_c3 = Probc3,
      PriorProb_c4 = Probc4, PriorProb_c5 = Probc5)
  } else {
...
cmargLCM5Chalfnorm_Eu <- function(object) {
  ...
  colnames(margEff_c2) <- paste0("Eu_", colnames(uHvar)[-1],
    "_c2")
  margEff_c3 <- kronecker(matrix(delta3[2:object$nuHvar], nrow = 1),
    matrix(exp(Wu3/2) * dnorm(0), ncol = 1))
  colnames(margEff_c3) <- paste0("Eu_", colnames(uHvar)[-1],
    "_c3")
  margEff_c4 <- kronecker(matrix(delta4[2:object$nuHvar], nrow = 1),
    matrix(exp(Wu4/2) * dnorm(0), ncol = 1))
  colnames(margEff_c4) <- paste0("Eu_", colnames(uHvar)[-1],
    "_c4")
  margEff_c5 <- kronecker(matrix(delta5[2:object$nuHvar], nrow = 1),
    matrix(exp(Wu5/2) * dnorm(0), ncol = 1))
...
cmargLCM5Chalfnorm_Vu <- function(object) {
  ...
  colnames(margEff_c2) <- paste0("Vu_", colnames(uHvar)[-1],
    "_c2")
  margEff_c3 <- kronecker(matrix(delta3[2:object$nuHvar], nrow = 1),
    matrix(exp(Wu3) * (1 - (dnorm(0)/pnorm(0))^2), ncol = 1))
  colnames(margEff_c3) <- paste0("Vu_", colnames(uHvar)[-1],
    "_c3")
  margEff_c4 <- kronecker(matrix(delta4[2:object$nuHvar], nrow = 1),
    matrix(exp(Wu4) * (1 - (dnorm(0)/pnorm(0))^2), ncol = 1))
  colnames(margEff_c4) <- paste0("Vu_", colnames(uHvar)[-1],
    "_c4")
  margEff_c5 <- kronecker(matrix(delta5[2:object$nuHvar], nrow = 1),
    matrix(exp(Wu5) * (1 - (dnorm(0)/pnorm(0))^2), ncol = 1))
