...
ctruncnormlike <- function(parm, nXvar, nmuHvar, nuHvar, nvHvar,
...
csttruncnorm <- function(olsObj, epsiRes, S, nmuHvar, nuHvar,
  uHvar, muHvar, nvHvar, vHvar) {
...
cgradtruncnormlike <- function(parm, nXvar, nmuHvar, nuHvar,
  nvHvar, muHvar, uHvar, vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuHvar)]
  delta <- parm[(nXvar + nmuHvar + 1):(nXvar + nmuHvar + nuHvar)]
  phi <- parm[(nXvar + nmuHvar + nuHvar + 1):(nXvar + nmuHvar +
...
chesstruncnormlike <- function(parm, nXvar, nmuHvar, nuHvar,
  nvHvar, muHvar, uHvar, vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  omega <- parm[(nXvar + 1):(nXvar + nmuHvar)]
  delta <- parm[(nXvar + nmuHvar + 1):(nXvar + nmuHvar + nuHvar)]
  ...
  hessll <- matrix(nrow = nXvar + nmuHvar + nuHvar + nvHvar,
    ncol = nXvar + nmuHvar + nuHvar + nvHvar)
  ...
  hessll[(nXvar + nmuHvar + 1):(nXvar + nmuHvar + nuHvar),
    ...
    MARGIN = 1, STATS = ((sigx13 - (((sigx10) * (mu * exp(Wv) -
      ...
      dmu)), FUN = "*"), uHvar)
  hessll[(nXvar + nmuHvar + nuHvar + 1):(nXvar + nmuHvar +
    nuHvar + nvHvar), (nXvar + nmuHvar + nuHvar + 1):(nXvar +
    nmuHvar + nuHvar + nvHvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = (sigx15 + dmusig * (mu/(sigx2) -
      ...
      exp(Wv), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nmuHvar), (nXvar + nmuHvar +
    1):(nXvar + nmuHvar + nuHvar)] <- crossprod(sweep(muHvar,
  ...
  hessll[(nXvar + 1):(nXvar + nmuHvar), (nXvar + nmuHvar +
    nuHvar + 1):(nXvar + nmuHvar + nuHvar + nvHvar)] <- crossprod(sweep(muHvar,
    MARGIN = 1, STATS = (((1/sigmastar - (muwuepsi) * (sigmu)/exp(Wu))/(sigma_sq) -
      sigx14 * exp(Wv)) * dmusig/pmusig + sigx8) * exp(Wv),
    FUN = "*"), vHvar)
  hessll[(nXvar + nmuHvar + 1):(nXvar + nmuHvar + nuHvar),
    (nXvar + nmuHvar + nuHvar + 1):(nXvar + nmuHvar + nuHvar +
      nvHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = (((sigx6) *
...
truncnormAlgOpt <- function(start, olsParam, dataTable, S, nXvar,
  muHvar, nmuHvar, uHvar, nuHvar, vHvar, nvHvar, Yvar, Xvar,
  ...
  mleObj$logL_OBS <- ctruncnormlike(parm = mleParam, nXvar = nXvar,
    nuHvar = nuHvar, nvHvar = nvHvar, nmuHvar = nmuHvar,
    muHvar = muHvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
    Xvar = Xvar, S = S)
  mleObj$gradL_OBS <- cgradtruncnormlike(parm = mleParam, nXvar = nXvar,
    nuHvar = nuHvar, nvHvar = nvHvar, nmuHvar = nmuHvar,
    muHvar = muHvar, uHvar = uHvar, vHvar = vHvar, Yvar = Yvar,
    Xvar = Xvar, S = S)
