...
craynormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
...
cstraynorm <- function(olsObj, epsiRes, S, nuHvar, uHvar, nvHvar,
  vHvar) {
...
cgradraynormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
...
chessraynormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
...
raynormAlgOpt <- function(start, olsParam, dataTable, S, nXvar,
  ...
  mleObj$logL_OBS <- craynormlike(parm = mleParam, nXvar = nXvar,
    nuHvar = nuHvar, nvHvar = nvHvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S)
  mleObj$gradL_OBS <- cgradraynormlike(parm = mleParam, nXvar = nXvar,
    nuHvar = nuHvar, nvHvar = nvHvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S)
