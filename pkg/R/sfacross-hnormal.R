...
chalfnormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
...
csthalfnorm <- function(olsObj, epsiRes, S, nuHvar, uHvar, nvHvar,
  vHvar) {
...
cgradhalfnormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
...
chesshalfnormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
  ...
  hessll[(nXvar + 1):(nXvar + nuHvar), (nXvar + 1):(nXvar +
    nuHvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = ((0.5/(sigma_sq)^2 +
