...
cLCMhalfnormlike4C <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  ...
  theta1 <- parm[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)]
  theta2 <- parm[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar +
    1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar)]
  theta3 <- parm[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 *
    nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 3 *
    nZvar)]
...
csLCMfhalfnorm4C <- function(olsObj, epsiRes, nXvar, nuHvar,
  ...
  StartVal <- c(Esti[1:(nXvar)], Esti[nXvar + 1], if (nuHvar >
    ...
    0.98 * Esti[1:(nXvar)], Esti[nXvar + 1], if (nuHvar >
      1) rep(0, nuHvar - 1), Esti[nXvar + 2], if (nvHvar >
      1) rep(0, nvHvar - 1), rep(0, 3 * nZvar))
...
cgradLCMhalfnormlike4C <- function(parm, nXvar, nuHvar, nvHvar,
...
chessLCMhalfnormlike4C <- function(parm, nXvar, nuHvar, nvHvar,
  uHvar, vHvar, Yvar, Xvar, S, Zvar, nZvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi1 <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
  ...
  hessll <- matrix(nrow = (4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    3 * nZvar), ncol = (4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    3 * nZvar))
  ...
  hessll[1:nXvar, (4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 *
    ...
    vHvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * sigx4w2z * sigx1_1 * ewz1/((sigma_sq1)^(3/2)),
    FUN = "*"), Zvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar +
    1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4wz * sigx1_1 * ewz1 *
      ewz2/((sigma_sq1)^(3/2))), FUN = "*"), Zvar)
  hessll[1:nXvar, (4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 *
    nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 3 *
  ...
  hessll[(nXvar + 1):(nXvar + nuHvar), (4 * nXvar + 4 * nuHvar +
    ...
    MARGIN = 1, STATS = -(4 * (prC * ewu1 * ewv4 * ewz1 *
      ...
      wzdsq1))), FUN = "*"), vHvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (4 * nXvar + 4 * nuHvar +
    4 * nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = sigx4w2z *
    ewu1 * ewz1 * sigx6_1/sqrt(sigma_sq1), FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (4 * nXvar + 4 * nuHvar +
    4 * nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 2 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(sigx4wz * ewu1 * ewz1 * ewz2 * sigx6_1/sqrt(sigma_sq1)),
    FUN = "*"), Zvar)
  hessll[(nXvar + 1):(nXvar + nuHvar), (4 * nXvar + 4 * nuHvar +
    4 * nvHvar + 2 * nZvar + 1):(4 * nXvar + 4 * nuHvar +
    4 * nvHvar + 3 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(sigx4w12z * ewu1 * ewz1 * ewz3 * sigx6_1/sqrt(sigma_sq1)),
    FUN = "*"), Zvar)
  ...
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (3 *
    ...
    nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (S * prC * sigx1_4 * (sigma_sq4) * ewv1 *
      ewz1 * sigx7_1 * sqrt(sigma_sq4)/(sigsq_4^2 * wzdsq1))),
    FUN = "*"), Xvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (4 *
    nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar + 4 *
    nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (prC * ewu4 * ewv1 * ewz1 * sigx7_1 * sigx6_4 *
      sqrt(sigma_sq4)/((sigwz4)^2 * wzdsq1))), FUN = "*"),
    uHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (4 *
    nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar + 4 *
    nuHvar + 4 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (prC * ewv1 * ewv4 * ewz1 * sigx7_1 * sigx7_4 *
      sqrt(sigma_sq4)/((sigwz4)^2 * wzdsq1))), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + 1):(4 * nXvar + 4 *
    nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx4w2z * ewv1 * ewz1 * sigx7_1/sqrt(sigma_sq1),
    FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + nZvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar + 2 * nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4wz * ewv1 * ewz1 * ewz2 *
      sigx7_1/sqrt(sigma_sq1)), FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar + 1):(4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar,
  ...
  hessll[(nXvar + nuHvar + nvHvar + 1):(2 * nXvar + nuHvar +
    ...
    vHvar)
  hessll[(nXvar + nuHvar + nvHvar + 1):(2 * nXvar + nuHvar +
    nvHvar), (4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4w3z * sigx1_2 * ewz1 *
      ewz2/((sigma_sq2)^(3/2))), FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + nvHvar + 1):(2 * nXvar + nuHvar +
    nvHvar), (4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar +
    1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * sigx4w4z * sigx1_2 * ewz2/((sigma_sq2)^(3/2)),
    FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + nvHvar + 1):(2 * nXvar + nuHvar +
    nvHvar), (4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar +
    1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 3 * nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4w12z * sigx1_2 * ewz2 *
  ...
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    ...
    1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx4w3z * ewu2 * ewz1 * ewz2 *
      sigx6_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 *
    nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = sigx4w4z *
    ewu2 * ewz2 * sigx6_2/sqrt(sigma_sq2), FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    2 * nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    3 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx4w12z *
  ...
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    ...
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4w3z * ewv2 * ewz1 * ewz2 *
      sigx7_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = sigx4w4z *
    ewv2 * ewz2 * sigx7_2/sqrt(sigma_sq2), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 2 * nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(sigx4w12z * ewv2 * ewz2 * ewz3 * sigx7_2/sqrt(sigma_sq2)),
    FUN = "*"), Zvar)
  ...
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4w3z * sigx1_3 * ewz1 *
      ewz3/((sigma_sq3)^(3/2))), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    sigx4wz * sigx1_3 * ewz2 * ewz3/((sigma_sq3)^(3/2))),
    FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 2 * nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 *
  ...
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    ...
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx4w3z * ewu3 * ewz1 * ewz3 *
      sigx6_3/sqrt(sigma_sq3)), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(sigx4wz *
  ...
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    ...
    STATS = sigx4w13z * ewu3 * ewz3 * sigx6_3/sqrt(sigma_sq3),
    FUN = "*"), Zvar)
  ...
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    ...
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4w3z * ewv3 * ewz1 * ewz3 *
      sigx7_3/sqrt(sigma_sq3)), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(sigx4wz *
  ...
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    ...
    nvHvar + 2 * nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = sigx4w13z * ewv3 * ewz3 * sigx7_3/sqrt(sigma_sq3),
    FUN = "*"), Zvar)
  ...
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    3 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * prC * sigx4w5z * sigx1_4 *
      ewz1/((sigma_sq4)^(3/2))), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    3 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    prC * sigx4w6z * sigx1_4 * ewz2/((sigma_sq4)^(3/2))),
    FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    3 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 2 * nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 *
  ...
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    ...
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(prC * sigx4w5z * ewu4 * ewz1 *
      sigx6_4/sqrt(sigma_sq4)), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(prC *
    sigx4w6z * ewu4 * ewz2 * sigx6_4/sqrt(sigma_sq4)), FUN = "*"),
    Zvar)
  hessll[(4 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 3 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 2 * nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1,
    STATS = -(prC * sigx4w14z * ewu4 * ewz3 * sigx6_4/sqrt(sigma_sq4)),
    FUN = "*"), Zvar)
  ...
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    ...
    nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(prC * sigx4w5z * ewv4 * ewz1 *
      sigx7_4/sqrt(sigma_sq4)), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(prC *
    sigx4w6z * ewv4 * ewz2 * sigx7_4/sqrt(sigma_sq4)), FUN = "*"),
    Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 3 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar), (4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 2 * nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 3 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(prC * sigx4w14z * ewv4 * ewz3 * sigx7_4/sqrt(sigma_sq4)),
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar + nZvar), (4 * nXvar + 4 * nuHvar +
    4 * nvHvar + 1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar +
    nZvar)] <- crossprod(sweep(Zvar, MARGIN = 1, STATS = ((1 -
    ewz1/wzdeno)/wzdsig - 2 * (dwp1/(wzdsig^2 * sqrt(sigma_sq1)))) *
    (2 * dpsq1 - sigx4) * ewz1, FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar + nZvar), (4 * nXvar + 4 * nuHvar +
    4 * nvHvar + nZvar + 1):(4 * nXvar + 4 * nuHvar + 4 *
    nvHvar + 2 * nZvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = -(((2 * dpsq1 - sigx4)/(sigx4wzsq) + 2 * ((2 *
      dpsq2 - sigx4) * depsisq1 * pmusig1/(wzdsig^2 * sqrt(sigma_sq1)))) *
      ewz1 * ewz2), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar + nZvar), (4 * nXvar + 4 * nuHvar +
    4 * nvHvar + 2 * nZvar + 1):(4 * nXvar + 4 * nuHvar +
    4 * nvHvar + 3 * nZvar)] <- crossprod(sweep(Zvar, MARGIN = 1,
    STATS = -(((2 * dpsq1 - sigx4)/(sigx4wzsq) + 2 * ((2 *
      dpsq3 - sigx4) * depsisq1 * pmusig1/(wzdsig^2 * sqrt(sigma_sq1)))) *
      ewz1 * ewz3), FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar + 1):(4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar), (4 * nXvar +
    4 * nuHvar + 4 * nvHvar + nZvar + 1):(4 * nXvar + 4 *
    nuHvar + 4 * nvHvar + 2 * nZvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = ((1 - ewz2/wzdeno)/wzdsig - 2 * (dwp2/(wzdsig^2 *
      sqrt(sigma_sq2)))) * (2 * dpsq2 - sigx4) * ewz2,
    FUN = "*"), Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + nZvar + 1):(4 *
    nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar), (4 * nXvar +
    4 * nuHvar + 4 * nvHvar + 2 * nZvar + 1):(4 * nXvar +
    4 * nuHvar + 4 * nvHvar + 3 * nZvar)] <- crossprod(sweep(Zvar,
    MARGIN = 1, STATS = -(((2 * dpsq2 - sigx4)/(sigx4wzsq) +
      2 * ((2 * dpsq3 - sigx4) * depsisq2 * pmusig2/(wzdsig^2 *
        sqrt(sigma_sq2)))) * ewz2 * ewz3), FUN = "*"),
    Zvar)
  hessll[(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar +
    1):(4 * nXvar + 4 * nuHvar + 4 * nvHvar + 3 * nZvar),
    (4 * nXvar + 4 * nuHvar + 4 * nvHvar + 2 * nZvar + 1):(4 *
      nXvar + 4 * nuHvar + 4 * nvHvar + 3 * nZvar)] <- crossprod(sweep(Zvar,
...
LCM4ChnormAlgOpt <- function(start, olsParam, dataTable, S, nXvar,
  ...
  start_st <- if (!is.null(start))
    start else csLCMfhalfnorm4C(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  if (initStart) {
    ...
    initModel <- nlminb(start = startVal, objective = function(parm) -sum(cLCMhalfnormlike4C(parm,
      ...
      S = S, Zvar = Zvar, nZvar = nZvar)), gradient = function(parm) -colSums(cgradLCMhalfnormlike4C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, Zvar = Zvar, nZvar = nZvar)), hessian = function(parm) -chessLCMhalfnormlike4C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
  ...
  mleObj <- switch(method, ucminf = ucminf(par = startVal,
    fn = function(parm) -sum(cLCMhalfnormlike4C(parm, nXvar = nXvar,
      ...
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, Zvar = Zvar,
      nZvar = nZvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike4C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    ...
    maxLikAlgo = maxRoutine(fn = cLCMhalfnormlike4C, grad = cgradLCMhalfnormlike4C,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike4C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike4C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike4C(parm,
        ...
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), hs = function(parm) as(-chessLCMhalfnormlike4C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike4C(parm,
        ...
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), hess = function(parm) -chessLCMhalfnormlike4C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
  ...
  mleObj$logL_OBS <- cLCMhalfnormlike4C(parm = mleParam, nXvar = nXvar,
    nuHvar = nuHvar, nvHvar = nvHvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S, Zvar = Zvar, nZvar = nZvar)
  mleObj$gradL_OBS <- cgradLCMhalfnormlike4C(parm = mleParam,
    nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar, uHvar = uHvar,
    vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, Zvar = Zvar,
    nZvar = nZvar)
...
cLCM4Chalfnormeff <- function(object, level) {
  ...
  phi4 <- object$mleParam[(4 * object$nXvar + 4 * object$nuHvar +
    3 * object$nvHvar + 1):(4 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar)]
  theta1 <- object$mleParam[(4 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar + 1):(4 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar + object$nZvar)]
  theta2 <- object$mleParam[(4 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar + object$nZvar + 1):(4 * object$nXvar +
    4 * object$nuHvar + 4 * object$nvHvar + 2 * object$nZvar)]
  theta3 <- object$mleParam[(4 * object$nXvar + 4 * object$nuHvar +
    4 * object$nvHvar + 2 * object$nZvar + 1):(4 * object$nXvar +
    4 * object$nuHvar + 4 * object$nvHvar + 3 * object$nZvar)]
