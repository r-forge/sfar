...
ctslnormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
  lambda <- parm[nXvar + nuHvar + nvHvar + 1]
...
csttslnorm <- function(olsObj, epsiRes, S, nuHvar, uHvar, nvHvar,
  vHvar) {
...
cgradtslnormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
...
chesstslnormlike <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  vHvar, Yvar, Xvar, S) {
  beta <- parm[1:(nXvar)]
  delta <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
  ...
  sigx5 <- 2 * ((dpa) * eA) - (sigx4) * eB
  pdbuv <- (eC) * pb - db * wvwu
  hessll <- matrix(nrow = nXvar + nuHvar + nvHvar + 1, ncol = nXvar +
    nuHvar + nvHvar + 1)
  ...
  hessll[1:nXvar, (nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)] <- crossprod(sweep(Xvar,
    ...
    FUN = "*"), vHvar)
  hessll[1:nXvar, (nXvar + nuHvar + nvHvar + 1)] <- matrix(colSums(sweep(Xvar,
...
tslnormAlgOpt <- function(start, olsParam, dataTable, S, nXvar,
  ...
  mleObj$logL_OBS <- ctslnormlike(parm = mleParam, nXvar = nXvar,
    nuHvar = nuHvar, nvHvar = nvHvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S)
  mleObj$gradL_OBS <- cgradtslnormlike(parm = mleParam, nXvar = nXvar,
    nuHvar = nuHvar, nvHvar = nvHvar, uHvar = uHvar, vHvar = vHvar,
    Yvar = Yvar, Xvar = Xvar, S = S)
...
cmargtslnorm_Eu <- function(object) {
  ...
  uHvar <- model.matrix(object$formula, data = object$dataTable,
    rhs = 2)
  lambda <- Wu <- as.numeric(crossprod(matrix(delta), t(uHvar)))
  margEff <- kronecker(matrix(delta[2:object$nuHvar] * (1 +
