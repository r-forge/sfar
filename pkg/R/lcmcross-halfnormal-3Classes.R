...
cLCMhalfnormlike3C <- function(parm, nXvar, nuHvar, nvHvar, uHvar,
  ...
  theta1 <- parm[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(3 *
    nXvar + 3 * nuHvar + 3 * nvHvar + nZvar)]
  theta2 <- parm[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar +
    1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 2 * nZvar)]
...
csLCMfhalfnorm3C <- function(olsObj, epsiRes, nXvar, nuHvar,
...
cgradLCMhalfnormlike3C <- function(parm, nXvar, nuHvar, nvHvar,
...
chessLCMhalfnormlike3C <- function(parm, nXvar, nuHvar, nvHvar,
  uHvar, vHvar, Yvar, Xvar, S, Zvar, nZvar) {
  beta1 <- parm[1:(nXvar)]
  delta1 <- parm[(nXvar + 1):(nXvar + nuHvar)]
  phi1 <- parm[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar)]
  ...
  hessll <- matrix(nrow = (3 * nXvar + 3 * nuHvar + 3 * nvHvar +
    2 * nZvar), ncol = (3 * nXvar + 3 * nuHvar + 3 * nvHvar +
    2 * nZvar))
  ...
  hessll[1:nXvar, (3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 *
    ...
    vHvar)
  hessll[1:nXvar, (3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(3 *
    nXvar + 3 * nuHvar + 3 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = S * sigx4w2z * sigx1_1 * ewz1/((sigma_sq1)^(3/2)),
    FUN = "*"), Zvar)
  hessll[1:nXvar, (3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar +
    1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 2 * nZvar)] <- crossprod(sweep(Xvar,
  ...
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (2 *
    ...
    STATS = -(4 * (S * prC * sigx1_3 * (sigma_sq3) * ewv1 *
      ewz1 * sigx7_1 * sqrt(sigma_sq3)/(sigsq_3^2 * wzdsq1))),
    FUN = "*"), Xvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (3 *
    nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar + 3 *
    nuHvar + 2 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (prC * ewu3 * ewv1 * ewz1 * sigx7_1 * sigx6_3 *
      sqrt(sigma_sq3)/((sigwz3)^2 * wzdsq1))), FUN = "*"),
    uHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (3 *
    nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar + 3 *
    nuHvar + 3 * nvHvar)] <- crossprod(sweep(vHvar, MARGIN = 1,
    STATS = -(4 * (prC * ewv1 * ewv3 * ewz1 * sigx7_1 * sigx7_3 *
      sqrt(sigma_sq3)/((sigwz3)^2 * wzdsq1))), FUN = "*"),
    vHvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (3 *
    nXvar + 3 * nuHvar + 3 * nvHvar + 1):(3 * nXvar + 3 *
    nuHvar + 3 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = sigx4w2z * ewv1 * ewz1 * sigx7_1/sqrt(sigma_sq1),
    FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + 1):(nXvar + nuHvar + nvHvar), (3 *
    nXvar + 3 * nuHvar + 3 * nvHvar + nZvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar + 2 * nZvar)] <- crossprod(sweep(vHvar,
  ...
  hessll[(nXvar + nuHvar + nvHvar + 1):(2 * nXvar + nuHvar +
    nvHvar), (3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(3 *
    nXvar + 3 * nuHvar + 3 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * sigx4w3z * sigx1_2 * ewz1 *
      ewz2/((sigma_sq2)^(3/2))), FUN = "*"), Zvar)
  hessll[(nXvar + nuHvar + nvHvar + 1):(2 * nXvar + nuHvar +
    nvHvar), (3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar +
    1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 2 * nZvar)] <- crossprod(sweep(Xvar,
  ...
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    ...
    1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(sigx4w3z * ewu2 * ewz1 * ewz2 *
      sigx6_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + nuHvar + nvHvar + 1):(2 * nXvar + 2 *
    nuHvar + nvHvar), (3 * nXvar + 3 * nuHvar + 3 * nvHvar +
    nZvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 2 *
  ...
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    ...
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(sigx4w3z * ewv2 * ewz1 * ewz2 *
      sigx7_2/sqrt(sigma_sq2)), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + nvHvar + 1):(2 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + nZvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar +
  ...
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar)] <- crossprod(sweep(Xvar,
    MARGIN = 1, STATS = -(S * prC * sigx4w5z * sigx1_3 *
      ewz1/((sigma_sq3)^(3/2))), FUN = "*"), Zvar)
  hessll[(2 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    2 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + nZvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(Xvar, MARGIN = 1, STATS = -(S *
    prC * sigx4w6z * sigx1_3 * ewz2/((sigma_sq3)^(3/2))),
    FUN = "*"), Zvar)
  ...
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    ...
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar)] <- crossprod(sweep(uHvar,
    MARGIN = 1, STATS = -(prC * sigx4w5z * ewu3 * ewz1 *
      sigx6_3/sqrt(sigma_sq3)), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 2 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 2 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + nZvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(uHvar, MARGIN = 1, STATS = -(prC *
    sigx4w6z * ewu3 * ewz2 * sigx6_3/sqrt(sigma_sq3)), FUN = "*"),
    Zvar)
  ...
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    ...
    nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar)] <- crossprod(sweep(vHvar,
    MARGIN = 1, STATS = -(prC * sigx4w5z * ewv3 * ewz1 *
      sigx7_3/sqrt(sigma_sq3)), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 2 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar), (3 * nXvar + 3 * nuHvar + 3 *
    nvHvar + nZvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar +
    2 * nZvar)] <- crossprod(sweep(vHvar, MARGIN = 1, STATS = -(prC *
    sigx4w6z * ewv3 * ewz2 * sigx7_3/sqrt(sigma_sq3)), FUN = "*"),
    Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(3 * nXvar +
    3 * nuHvar + 3 * nvHvar + nZvar), (3 * nXvar + 3 * nuHvar +
    3 * nvHvar + 1):(3 * nXvar + 3 * nuHvar + 3 * nvHvar +
  ...
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + 1):(3 * nXvar +
    ...
    STATS = -(((2 * dpsq1 - sigx4)/(sigx4wzsq) + 2 * ((2 *
      ...
      ewz1 * ewz2), FUN = "*"), Zvar)
  hessll[(3 * nXvar + 3 * nuHvar + 3 * nvHvar + nZvar + 1):(3 *
    nXvar + 3 * nuHvar + 3 * nvHvar + 2 * nZvar), (3 * nXvar +
    3 * nuHvar + 3 * nvHvar + nZvar + 1):(3 * nXvar + 3 *
    nuHvar + 3 * nvHvar + 2 * nZvar)] <- crossprod(sweep(Zvar,
...
LCM3ChnormAlgOpt <- function(start, olsParam, dataTable, S, nXvar,
  ...
  start_st <- if (!is.null(start))
    start else csLCMfhalfnorm3C(olsObj = olsParam, epsiRes = dataTable[["olsResiduals"]],
  ...
  if (initStart) {
    ...
    initModel <- nlminb(start = startVal, objective = function(parm) -sum(cLCMhalfnormlike3C(parm,
      ...
      S = S, Zvar = Zvar, nZvar = nZvar)), gradient = function(parm) -colSums(cgradLCMhalfnormlike3C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      S = S, Zvar = Zvar, nZvar = nZvar)), hessian = function(parm) -chessLCMhalfnormlike3C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
  ...
  mleObj <- switch(method, ucminf = ucminf(par = startVal,
    fn = function(parm) -sum(cLCMhalfnormlike3C(parm, nXvar = nXvar,
      ...
      vHvar = vHvar, Yvar = Yvar, Xvar = Xvar, S = S, Zvar = Zvar,
      nZvar = nZvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike3C(parm,
      nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
      uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
    ...
    maxLikAlgo = maxRoutine(fn = cLCMhalfnormlike3C, grad = cgradLCMhalfnormlike3C,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike3C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), gr = function(parm) -colSums(cgradLCMhalfnormlike3C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike3C(parm,
        ...
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), hs = function(parm) as(-chessLCMhalfnormlike3C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
      ...
      fn = function(parm) -sum(cLCMhalfnormlike3C(parm,
        ...
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
        S = S, Zvar = Zvar, nZvar = nZvar)), hess = function(parm) -chessLCMhalfnormlike3C(parm,
        nXvar = nXvar, nuHvar = nuHvar, nvHvar = nvHvar,
        uHvar = uHvar, vHvar = vHvar, Yvar = Yvar, Xvar = Xvar,
...
cLCM3Chalfnormeff <- function(object, level) {
  ...
  phi3 <- object$mleParam[(3 * object$nXvar + 3 * object$nuHvar +
    2 * object$nvHvar + 1):(3 * object$nXvar + 3 * object$nuHvar +
    3 * object$nvHvar)]
  theta1 <- object$mleParam[(3 * object$nXvar + 3 * object$nuHvar +
    3 * object$nvHvar + 1):(3 * object$nXvar + 3 * object$nuHvar +
    3 * object$nvHvar + object$nZvar)]
  theta2 <- object$mleParam[(3 * object$nXvar + 3 * object$nuHvar +
    3 * object$nvHvar + object$nZvar + 1):(3 * object$nXvar +
    3 * object$nuHvar + 3 * object$nvHvar + 2 * object$nZvar)]
  ...
  Pcond_c3 <- Pi3 * Probc3/(Pi1 * Probc1 + Pi2 * Probc2 + Pi3 *
    Probc3)
  Group_c <- which.max(c(Pcond_c1, Pcond_c2, Pcond_c3))
  ...
  if (object$logDepVar == TRUE) {
    ...
    res <- bind_cols(u_c = u_c, teJLMS_c = teJLMS_c, PosteriorProb_c = P_cond_c,
      ...
      PosteriorProb_c3 = Pcond_c3, PriorProb_c1 = Probc1,
      PriorProb_c2 = Probc2, PriorProb_c3 = Probc3)
  } else {
