...
cstweibullnorm <- function(olsObj, epsiRes, S, nuHvar, uHvar,
  nvHvar, vHvar) {
...
cgradweibullnormlike <- function(parm, nXvar, nuHvar, nvHvar,
  ...
  Wv <- as.numeric(crossprod(matrix(phi), t(vHvar)))
  epsilon <- Yvar - as.numeric(crossprod(matrix(beta), t(Xvar)))
  gradll <- matrix(nrow = N, ncol = nXvar + nuHvar + nvHvar +
    1)
...
weibullnormAlgOpt <- function(start, olsParam, dataTable, S,
  nXvar, N, FiMat, uHvar, nuHvar, vHvar, nvHvar, Yvar, Xvar,
...
cweibullnormeff <- function(object, level) {
  ...
  k <- object$mleParam[object$nXvar + object$nuHvar + object$nvHvar +
    1]
...
cmargweibull_Eu <- function(object) {
  ...
  uHvar <- model.matrix(object$formula, data = object$dataTable,
    rhs = 2)
  Wu <- as.numeric(crossprod(matrix(delta), t(uHvar)))
  margEff <- kronecker(matrix(delta[2:object$nuHvar] * 1/2,
